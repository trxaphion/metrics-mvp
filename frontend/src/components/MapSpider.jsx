...
class MapSpider extends Component {
    ...
    if (hoverRoute) {
      ...
      if (spiderSelection.stops.length) {
        routeStops = routeStops.filter(stop =>
          spiderSelection.stops.find(spiderStop =>
    ...
    const items = selectedStops.map(startMarker => {
      ...
      if (hoverRoute) {
        ...
        waitScaled = stats.waitRankCount
          ? Math.trunc((1 - 1 / stats.waitRankCount) * 3) + 2
          : 4;
      } else {
        waitScaled = stats.waitRankCount
  ...
  generateTerminalCircle = (startMarker, waitScaled) => {
    ...
    const routeColor = this.routeColor(startMarker.routeIndex % 10);
    const opacity = this.props.hoverRoute ? 1 : 0.75;
  ...
  generatePolyline = (startMarker, waitScaled, i) => {
    ...
    const opacity = this.props.hoverRoute ? 0.6 : 0.5;
